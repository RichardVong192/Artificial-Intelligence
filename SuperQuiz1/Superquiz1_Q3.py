...
class RoutingGraph(Graph):
    ...
    def estimated_cost_to_goal(self, node):
        ...
        move_cost = 5   #Each move cost is 5
        costs = []      #List of costs
        for goal in self.goal_nodes:
            node_movement = (abs(goal[0] - node[0]) + abs(goal[1] - node[1]))   #Calculate the movement cost from starting to goal node
            costs.append(node_movement * move_cost)                             #Calculate the cost
        return min(costs)                                                       #We are only interested in the shortest time to reach the goal
...
def print_map(map_graph, frontier, solution):
    ...
    if solution != None:        #Only if there is a solution
        for arc in solution:
            row = arc.head[0]   
            col = arc.head[1]   
            if map_graph.list_map[row][col] != "S" and map_graph.list_map[row][col] != "G":
                map_graph.list_map[row][col] = "*"

    for path in frontier.expanded_nodes:
        row = path[0]   
        col = path[1]
        if map_graph.list_map[row][col] == " ":
            map_graph.list_map[row][col] = "."
    
    for item in map_graph.list_map:
        print("".join(item))    
